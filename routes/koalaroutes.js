...
router.get("/flights/:searchId", authMiddleware, async (req, res) => {
  try {
    ...
    let attempts = 0;
    const maxAttempts = 40; // up to 2 minutes polling
    let flightsArray = [];

    while (attempts < maxAttempts && flightsArray.length === 0) {
      ...
      flightsArray = Array.isArray(resultsData.proposals) ? resultsData.proposals : [];
    ...
    res.json({ status: "pending", message: "Results not ready yet, try again shortly.", proposals: [] });
